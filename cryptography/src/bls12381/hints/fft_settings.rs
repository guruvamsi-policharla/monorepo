...
pub struct Settings {
    pub max_width: usize,
    pub root_of_unity: Scalar,
    pub expanded_roots_of_unity: Vec<Scalar>,
    pub reverse_roots_of_unity: Vec<Scalar>,
...
impl Settings {
    ...
    pub fn new(scale: usize) -> Result<Settings, String> {
        ...
        let root_of_unity = Scalar::from_u64_arr(&SCALE2_ROOT_OF_UNITY[scale]);
    ...
    pub fn fft(&self, data: &[Scalar], inverse: bool) -> Result<Vec<Scalar>, String> {
        ...
        let mut output = vec![Scalar::zero(); data.len()];
        ...
        fft_fr_fast(&mut output, data, 1, roots, stride);

        if inverse {
            let inv_fr_len = Scalar::from_u64(data.len() as u64).inv();
            output
                .iter_mut()
                .for_each(|f| *f = f.clone().mul(&inv_fr_len));
        }

        Ok(output)
    }

    pub fn fft_g1(&self, data: &[G1], inverse: bool) -> Result<Vec<G1>, String> {
        if data.len() > self.max_width {
            return Err(String::from(
                "Supplied list is longer than the available max width",
            ));
        ...
        let mut output = vec![G1::zero(); data.len()];
        ...
        fft_g1_fast(&mut output, data, 1, roots, stride);

        if inverse {
            let inv_fr_len = Scalar::from_u64(data.len() as u64).inv();
            output
                .iter_mut()
                .for_each(|g| *g = g.clone().mul(&inv_fr_len));
        }
...
pub fn expand_root_of_unity(root: &Scalar, width: usize) -> Result<Vec<Scalar>, String> {
    let mut generated_powers = vec![Scalar::one(), root.clone()];

    while !(generated_powers.last().unwrap() == &Scalar::one()) {
...
pub fn fft_fr_fast(
    ...
    if half > 0 {
        ...
        for i in 0..half {
            let y_times_root = ret[i + half].clone().mul(&roots[i * roots_stride]);
            ret[i + half] = ret[i].clone().sub(&y_times_root);
            ret[i] = ret[i].clone().add(&y_times_root);
        }
    } else {
