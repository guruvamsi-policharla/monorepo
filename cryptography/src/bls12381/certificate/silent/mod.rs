//! BLS12-381 threshold signing scheme with silent setup
//!
//! This module provides both the generic BLS12-381 silent threshold implementation and a macro to generate
...
/// Generic BLS12-381 silent-threshold implementation.
///
    ...
    fn read_cfg(reader: &mut impl Buf, participants: &usize) -> Result<Self, Error> {
        ...
        if signers.count() == 0 {
            return Err(Error::Invalid(
                "cryptography::bls12381::certificate::silent::Certificate",
...
mod macros {
    ...
    /// # Example
    /// ```ignore
    /// impl_certificate_bls12381_silent!(VoteSubject<'a, D>);
    /// ```
    #[macro_export]
    macro_rules! impl_certificate_bls12381_silent {
        ($subject:ty) => {
            /// Generates a test fixture with Ed25519 identities and BLS12-381 silent-threshold schemes.
            ///
            ...
            {
                $crate::bls12381::certificate::silent::mocks::fixture::<_, V, _>(
                    rng,
                    n,
                    Scheme::signer,
                    Scheme::verifier,
            ...
            pub struct Scheme<
                ...
                V: $crate::bls12381::primitives::variant::Variant,
            > {
                generic: $crate::bls12381::certificate::silent::Generic<P, V>,
            }
            ...
            > Scheme<P, V> {
                ...
                pub fn signer(
                    ...
                    Some(Self {
                        generic: $crate::bls12381::certificate::silent::Generic::signer(
                            participants,
                            private_key,
                        )?,
                    })
                ...
                pub const fn verifier(
                    ...
                    Self {
                        generic: $crate::bls12381::certificate::silent::Generic::verifier(
                            participants,
                        ),
                    }
                }
            ...
            > $crate::certificate::Scheme for Scheme<P, V> {
                ...
                type Certificate = $crate::bls12381::certificate::silent::Certificate<V>;
                ...
                fn certificate_codec_config_unbounded() -> <Self::Certificate as commonware_codec::Read>::Cfg {
                    $crate::bls12381::certificate::silent::Generic::<P, V>::certificate_codec_config_unbounded()
                }
            }
        };
    }
...
mod tests {
    ...
    use crate::{
        ...
        impl_certificate_bls12381_silent,
    ...
    const NAMESPACE: &[u8] = b"test-bls12381-silent";
    ...
    impl_certificate_bls12381_silent!(TestSubject<'a>);
